...
public class FirewallManagerTests(ITestOutputHelper outputHelper)
{
    ...
    {
        var manager = GetFirewallManager();
        var networks = await manager.GetNetworksAsync("one.one.one.one", TestContext.Current.CancellationToken);
        Assert.NotNull(networks);
        Assert.Contains(IPNetwork2.Parse("1.1.1.1/32"), networks);
    ...
    public async Task GetKnownRulesAsync_Works()
    {
        ...
        var knownFqdn = new KnownFirewallRuleFqdn("MAX_HOME", "one.one.one.one");
        ...
        Assert.Contains(rules, r => r.Network == IPNetwork2.Parse("1.1.1.1/32"));
        Assert.All(rules, r => Assert.StartsWith("MAX_HOME", r.Name));
        ...
        Assert.Contains(rules, r => r.Name.StartsWith("MAX_HOME") && r.Network == IPNetwork2.Parse("1.1.1.1/32"));
        Assert.Contains(rules, r => r.Name.StartsWith("YOUR_HOME_") && r.Network == IPNetwork2.Parse("104.237.62.213/32"));
        Assert.Contains(rules, r => r.Name.StartsWith("YOUR_HOME_") && r.Network == IPNetwork2.Parse("173.231.16.77/32"));
        Assert.Contains(rules, r => r.Name.StartsWith("YOUR_HOME_") && r.Network == IPNetwork2.Parse("2607:f2d8:1:3c::3/128"));
        Assert.Contains(rules, r => r.Name.StartsWith("YOUR_HOME_") && r.Network == IPNetwork2.Parse("2607:f2d8:4010:51::5/128"));
    }
